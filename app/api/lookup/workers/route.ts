...
/** Trabalhadores aprovados e ativos (id e nome) para campos de seleção dos módulos. */
export async function GET(request: Request) {
  try {
    const actor = await requireActor();
    const { department } = schema.parse(Object.fromEntries(new URL(request.url).searchParams));
    const allowed = department ? await hasPermission(actor, "department", "read", department) : await hasAnyDepartmentPermission(actor, "read");
    ...
    const result = await query<{ id: string; name: string; departments: string[] }>(
      `select w.id, w.full_name as name, coalesce(array_agg(d.department_key order by d.department_key) filter (where d.department_key is not null), '{}') as departments
        ...
        where w.status = 'active' and ($1::text is null or exists (select 1 from app.worker_departments x where x.worker_id = w.id and x.department_key = $1))
